...
```sh
src-git feed-prplMesh https://gitlab.com/prpl-foundation/prplOS/feed-prpl.git
```
Now to add the packages on your prpl feed to your OpenWrt/LEDE instance:
```sh
./scripts/feeds update feed-prplMesh #retrieve the prpl feed from service/update to latest
./scripts/feeds install -p feed-prplMesh #make all of the prpl feed packages available to the build
```
...
```sh
src-link feed-prplMesh /full/path/to/feed-prpl/root
```
